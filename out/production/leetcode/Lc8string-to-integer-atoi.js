...
function myAtoi(s) {
    ...
    let min = 0 - Math.pow(2, 31), max = Math.pow(2, 31) - 1;
    let set = new Set();
    set.add('+');
    set.add('-');
    for (let i = 0; i < l; i++) {
        if (numStrArr.indexOf(s[i]) < 0 && !set.has(s[i]))
            break;
        if (s[i] == ' ') {
            continue;
        }
    ...
    for (let i = 0; i < nl; i++) {
        if (set.has(numStr[nl - i - 1])) {
            ...
            res += numStr[nl - i - 1] * (Math.pow(10, i));
        }
    }
    if (res < min)
        return min;
...
//# sourceMappingURL=Lc8string-to-integer-atoi.js.map